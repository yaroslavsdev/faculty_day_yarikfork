...
```bash
ls logs -S | head -n 1 > biglog.txt
```
-S - флаг для сортировки по убыванию
...
```bash
zip -r project_backup.zip $(find logs/config -type f -name "*.conf") $(find logs -type f -name "*.log") errors.txt
```
...
```bash
wc -l logs/config/*.conf > conf_stats.txt
```
